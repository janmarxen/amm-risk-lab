...
from python.utils.zero_preserving_scaler import ZeroPreservingStandardScaler
import random
...
from python.utils.data_utils import get_X_y
...
def fit_scalers(
    ...
    Returns:
        (feature_scaler, target_reg_scalers): Fitted scalers - StandardScaler for features, 
                                             ZeroPreservingStandardScaler for liquidity_return (Task 1), 
                                             StandardScaler for volume_return (Task 2).
    """
    if len(targets) != 2:
    ...
    if target_reg_scalers is None:
        # Use zero-preserving scaler for liquidity_return (Task 1) and standard scaler for volume_return (Task 2)
        target_reg_scalers = [ZeroPreservingStandardScaler(), StandardScaler()]
    scaler_lock = Lock()
...
class LPsDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            target_reg_scalers: List of 2 fitted target scalers (ZeroPreservingStandardScaler for liquidity_return, StandardScaler for volume_return).
    ...
    def _load_all_data(self, num_workers):
        ...
        def load_pool(pool_addr):
            try:
                ...
                if self.feature_scaler is not None:
                    ...
                    # Use zero-preserving scaler for liquidity_return (Task 1) and standard scaler for volume_return (Task 2)
                    temp_target_scaler_1 = ZeroPreservingStandardScaler()
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
            ...
            for i, future in enumerate(futures, 1):
                X_pool, y_cls_1_pool, y_reg_1_pool, y_reg_2_pool = future.result()
                if len(X_pool) > 0:
